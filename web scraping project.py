...
# # Last 10 Years History of Bitcoin
...
bitcoin_df['Price_in_dollar']=bitcoin_df.Price_in_dollar.replace('$','').replace(',','').astype(float)
bitcoin_df['Marketcap_in_dollar']=bitcoin_df.Marketcap_in_dollar.replace('$','').replace(',','').astype(float)
bitcoin_df['1hour_chg_per']=bitcoin_df['1hour_chg_per'].replace('%','').replace(',','').astype(float)
bitcoin_df['24hour_chg_per']=bitcoin_df['24hour_chg_per'].replace('%','').replace(',','').astype(float)
bitcoin_df['7day_chg_per']=bitcoin_df['7day_chg_per'].replace('%','').replace(',','').astype(float)
bitcoin_df['Circulating_supply_in_BTC']=bitcoin_df['Circulating_supply_in_BTC'].str.replace('BTC','')
...
bitcoin1 = pd.read_csv(r'C:\DATA\bitcoin.csv')
...
sns.displot(data=bitcoin,x=bitcoin.Price_in_dollar,height=5,aspect=13/5,color='red',kde = T
